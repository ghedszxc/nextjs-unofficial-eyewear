...
export function ProductCard(props: ProductCardProps) {
  ...
  return (
    <div
      className={`bg-secondary-light flex size-full flex-col justify-between border-r border-b border-black ${
        props.className ?? ""
      } ${props.lastInRow ? "border-b border-black" : ""}`}
      onMouseEnter={() => setIsHovering(true)}
      onMouseLeave={() => setIsHovering(false)}
    >
      <Carousel setApi={setApi} className="flex flex-col justify-between py-6 lg:px-4 lg:py-10">
      ...
      <div className="flex flex-col gap-2 border-t border-black p-4 lg:h-[101px] lg:gap-4 lg:p-6">
        <p className="font-gt-america-expanded-bold text-xs lg:h-[21px] lg:text-base">{active?.name ?? ""}</p>
        <div className="flex justify-between lg:h-4 lg:uppercase">
